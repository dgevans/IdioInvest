...
def calibrate(Parahat):
    global F,G,f,ny,ne,nY,nz,nv,n,Ivy,Izy,IZY,nG,n_p,nZ,neps,k
...
class approximate(object):
    ...
    def approximate_Gamma(self):
        ...
        cluster = Para.nomalize(cluster,weights)        
    ...
    def compute_d2y_Eps(self):
        ...
        def temp(z_i):
            DFi,df = DF(z_i)[n:],self.df(z_i)
            return -np.linalg.inv(DFi[:,y]+DFi[:,e].dot(df)+DFi[:,v].dot(Ivy).dot(self.dy[z](z_i)).dot(Izy))  
        temp = dict_fun(temp)
        
        def A2_i(z_i): #the pure term
            DFi = DF(z_i)[n:]
            df,hf = self.df(z_i),self.Hf(z_i)
            return np.tensordot(temp(z_i),np.tensordot(DFi[:,e].dot(df),self.d2y[Eps,Eps](z_i),1)
            +np.tensordot(DFi[:,e],quadratic_dot(hf,self.dy[Eps](z_i),self.dy[Eps](z_i)),1)
            ,axes=1)
        A2_i = dict_fun(A2_i)
        B2_i = dict_fun(lambda z_i : temp(z_i).dot(DF(z_i)[n:,Y] + DF(z_i)[n:,v].dot(Ivy).dot(self.dy[Z](z_i)).dot(IZY) ))
        C2_i = dict_fun(lambda z_i : temp(z_i).dot(DF(z_i)[n:,v].dot(Ivy).dot(self.dy[Y](z_i))) )
        ...
        d2y_sigmaE =lambda z_i: A2_i(z_i) + np.tensordot(C2_i(z_i).dot(tempC),A2,1)
    ...
    def compute_d2y(self):
        ...
        def d2y_YSZ(z_i):
            ...
            return - np.tensordot(temp, DFi[:,v].dot(Ivy).dot(self.dy[Y,S,Z](z_i)),1)
