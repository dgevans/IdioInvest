...
ny = 20 # number of individual controls (m_{t},mu_{t},c_{t},l_{t},rho1_,rho2,phi,x_{t-1},kappa_{t-1}) Note that the forward looking terms are at the end
...
def F(w):
    ...
    '''
    logm,muhat,nu_a,logc,lognl,logl,phi,phi2,foc_k,r,rho2_,rho3_,res,foc_R_,foc_tau_k,Efoc_tau_k,x_,logk_,rho1_,alpha_ = w[:ny] #y
    EUc,EUc_r,Ex_,Ek_,EUc_mu,Erho2_,Erho3_,Efoc_k,hatEfoc_tau_k= w[ny:ny+ne] #e
    logXi,tau_k,logAlpha,logK_,R_,W,Eta,tau_l,T = w[ny+ne:ny+ne+nY] #Y
    ...
    ret[20] = Efoc_k #k_
    ret[21] = mu_  - EUc_mu/EUc 
    ret[22] = Alpha_ - beta*R_*m_*EUc #rho2_
    ret[23] = Alpha_ - beta*m_*EUc_r #rho3
...
def G(w):
    ...
    '''
    logm,muhat,nu_a,logc,lognl,logl,phi,phi2,foc_k,r,rho2_,rho3_,res,foc_R_,foc_tau_k,Efoc_tau_k,x_,logk_,rho1_,alpha_ = w[:ny] #y
    logXi,tau_k,logAlpha_,logK_,R_,W,Eta,tau_l,T = w[ny+ne:ny+ne+nY] #Y
    logm_,muhat_,nu_a_= w[ny+ne+nY:ny+ne+nY+nz] #z
    logXi,tau_k_ = w[ny+ne+nY+nz+nv+n_p:ny+ne+nY+nz+nv+n_p+nZ] #Z
    eps_p,eps_t= w[ny+ne+nY+nz+nv+n_p+nZ:ny+ne+nY+nz+nv+n_p+nZ+neps] #shock
    Eps = w[ny+ne+nY+nz+nv+n_p+nZ+neps] #aggregate shock
...
def f(y):
    ...
    '''
    logm,muhat,nu_a,logc,lognl,logl,phi,phi2,foc_k,r,rho2_,rho3_,res,foc_R_,foc_tau_k,Efoc_tau_k,x_,logk_,rho1_,alpha_ = y
...
def Finv(YSS,z):
    ...
    '''
    logm,muhat,nu_a = z
    logXi,tau_k,logAlpha,logK_,R_,W,Eta,tau_l,T = YSS
    ...
    return np.vstack((
    logm,muhat,nu_a,np.log(c),np.log(nl),np.log(l),phi,phi2,foc_k,r,rho2,rho3,res,foc_R,foc_tau_k,foc_tau_k,x_,np.log(k_),rho1,alpha
    ))
...
def GSS(YSS,y_i,weights):
    ...
    '''
    logm,muhat,nu_a,logc,lognl,logl,phi,phi2,foc_k,r,rho2_,rho3_,res,foc_R_,foc_tau_k,Efoc_tau_k,x_,logk_,rho1_,alpha_   = y_i
    logXi,tau_k,logAlpha,logK_,R_,W,Eta,tau_l,T = YSS
