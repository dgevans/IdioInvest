...
def F(w):
    ...
    '''
    logm,muhat,nu_a,nu_e,logc,logl,lognl,w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2_,rho3_,b_,labor_res,res,foc_W,zeta,x_,rho2hat_,rho3hat_,lamb_,logk_,alpha_ = w[:ny] #y
    EUc,EUc_r,Ex_,Ek_,EUc_mu,Erho2_,Erho3_,Efoc_k,Efoc_tau_k= w[ny:ny+ne] #e
    logXi,logAlpha_,logK_,R_,W,tau_l,tau_k,Eta,T,Zeta,B_ = w[ny+ne:ny+ne+nY] #Y
    ...
    ret[4] = x_*Uc/(beta*EUc) + Uc*((1-tau_k)*(pi-k_) - (Ri_-1) *k_)  +(1-tau_l)*W*w_e*l*Uc - Uc*(c-T) - x #x
    ...
    ret[12] = (Uc + Ucc*x_/(beta*EUc)*(mu-mu_) + mu*Ucc*( (1-tau_k)*(pi-k_)-(Ri_-1.)*k_  + (1-tau_l)*W*w_e*l-(c-T) -Uc/Ucc )
    ...
    ret[28] = zeta -Zeta
    ret[29] = alpha_ - Alpha_
    
    ret[30] = Efoc_k #k_
    ret[31] = mu_  - EUc_mu/EUc - m_*lamb_/Alpha_#muhat
    ret[32] = Alpha_ - beta*Ri_*m_*EUc #rho2_
    ret[33] = Alpha_*(1-1/Ri_) - beta*(1-tau_k)*m_*EUc_r #rho3
...
def f(y):
    ...
    '''
    logm,muhat,nu_a,nu_e,logc,logl,lognl,w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2_,rho3_,b_,labor_res,res,foc_W,zeta,x_,rho2hat_,rho3hat_,lamb_,logk_,alpha_ = y
    ...
    ret[6] = rho3_
    ret[7] = foc_k   
    ret[8] = (pi-k_)*mu*Uc + rho3_/beta + pi*zeta
...
def Finv(YSS,z):
    ...
    logXi,logAlpha,logK_,R_,W,tau_l,tau_k,Eta,T,Zeta,B_ = YSS
    ...
    foc_tau_k = (pi-k_)*mu*Uc + rho3/beta + pi*zeta
    res = (f - c - Gov - k_)
    ...
    return np.vstack((
    logm,muhat,nu_a,nu_e,np.log(c),np.log(l),np.log(nl),w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2,rho3,b_,  labor_res,res,foc_W,zeta,x_,rho2hat_,rho3hat_,lamb,np.log(k_),alpha_   
    ))
...
def GSS(YSS,y_i,weights):
    ...
    '''
    logm,muhat,nu_a,nu_e,logc,logl,lognl,w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2_,rho3_,b_,labor_res,res,foc_W,zeta,x_,rho2hat_,rho3hat_,lamb_,logk_,alpha_  = y_i
    logXi,logAlpha,logK_,R_,W,tau_l,tau_k,Eta,T,Zeta,B_ = YSS
