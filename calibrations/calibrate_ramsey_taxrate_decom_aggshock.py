...
def F(w):
    ...
    ret[4] = x_*Uc/(beta*EUc)  + Uc*((1-tau_k_)*(pi-k_) - (R_-1) *k_)  +(1-tau_l)*W*w_e*l*Uc - Uc*(c-T) - x #x
    ...
    ret[12] = (Uc + Ucc*x_/(beta*EUc)*(mu-mu_) + mu*Ucc*( (1-tau_k_)*(pi-k_)-(R_-1.)*k_  + (1-tau_l)*W*w_e*l-(c-T) -Uc/Ucc )
    ...
    ret[14] = foc_k - ( mu*Uc*((1-tau_k_)*(pi_k-1) - (R_-1)) + (1-tau_k_)*Uc*r_k/EUc_r *rho3_/beta 
    ...
    ret[19] = foc_R - (-beta*EUc_mu*k_ - (1-tau_k_)*rho3_/(R_*(R_-1))+ rho2_ ) #foc_alpha2
    ...
    ret[28] = tR_khat - mu*Uc*( (pi-k_) - dk_dtau_k * ((1-tau_k_)*(pi_k-1) - (R_-1)))*(1-tau_k_)/((R_-1)*dK_dtau_k*EXi)
    ret[29] = tW_khat - (dk_dtau_k * fnk * phi2) *(1-tau_k_)/((R_-1)*dK_dtau_k*EXi)
    ret[30] = tE_khat - (dK_dtau_k * (Xi_/beta - EXi * R_) + dk_dtau_k*cov_Uc_r*EXi/EUc - dk_dtau_k*covXi_r)*(1-tau_k_)/((R_-1)*dK_dtau_k*EXi)
    ret[31] = xi - Xi
    ...
    ret[43] = Alpha_*(1-1/R_) - beta*(1-tau_k_)*m_*EUc_r #rho3
