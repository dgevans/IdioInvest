...
def F(w):
    ...
    ret[4] = x_*Uc/(beta*EUc) + Uc*((1-tau_k_)*((1-xi_l)*f+(1-delta_i)*k_) - R_ *k_) + Uc*W*(1-tau_l)*nl - Uc*(c-T) - x #x
    ...
    ret[18] = foc_tau_k - (m_*rho3_*(fk+(1-delta_i))*Uc - mu*Uc*((1-xi_l)*f+(1-delta_i)*k_) )
    ret[19] = Efoc_tau_k - hatEfoc_tau_k
    ret[20] = xi - Xi
    ret[21] = dk_dtau_k_ - ( EUcfk/((1-tau_k_)*EUcfkk) )
    ret[22] = tR_khat - mu*Uc*((1-xi_l)*f +(1- delta_i)*k_ - dk_dtau_k_ *( (1-tau_k_)*((1-xi_l)*fk+(1-delta_i)) - R_  ) )*(1-tau_k_)/((R_)*dK_dtau_k_*EXi)
...
def Finv(YSS,z):
    ...
    T1 = ((1/beta)/(1-tau_k) + delta_i-1)/ ( A*xi_k *(W/(A*xi_l))**((xi_k-1)/xi_k))
    T2 = (xi_k+xi_l - 1.)/xi_k
    l = T1**(1/T2)
    ...
    x_ = -( Uc*((1-tau_k)*((1-xi_l)*f+(1-delta_i)*k_) - R_ *k_) +Uc*(1-tau_l)*W*nl - Uc*(c-T) )/(1/beta-1)
        
    phi2 = -(Un + mu*Uc*W*(1-tau_l) + Eta)*W*Uc/Unn
    
    rho3 = (Uc*mu*((1-tau_k)*((1-xi_l)*(fk-fl*flk/fll)+1-delta_i) - 1/beta) + Eta*flk/fll - Xi/beta + ((fk-fl*flk/fll)+1-delta_i)*Xi  ) / (m*(1-tau_k)*Uc*(fkk-flk*flk/fll))
    ...
    foc_tau_k = (m*rho3*(fk+1-delta_i)*Uc - mu*Uc*((1-xi_l)*f+(1-delta_i)*k_) )
    
    xi = Xi*np.ones(c.shape)
    dk_dtau_k_ = ( (fk+1-delta_i)/((1-tau_k)*fkk) )
    tR_k = mu*Uc*((1-xi_l)*f +(1- delta_i)*k_ - dk_dtau_k_ *( (1-tau_k)*((1-xi_l)*fk+1-delta_i) - R_  ) )*(1-tau_k)/((R_)*dK_dtau_k_*Xi)
