...
def F(w):
    ...
    '''
    logm,muhat,nu_a,logc,logl,r,rho2_,rho3_,phi,res,foc_k,foc_Tau_k,foc_Tau_k_,foc_R,foc_R_,xi,tauR_k,tauR_k_,tauR_l,x_,logk_,rho1_,alpha_ = w[:ny] #y
    EUc,EUc_r,EUc_mu,Efoc_k_,Efoc_Tau_k,Efoc_R,EXi,EtauR_k= w[ny:ny+ne] #e
    logXi,Tau_k,logAlpha_,logK_,R_,W,Tau_l,Psi,T,TauR_k_,YUc,TauR_l = w[ny+ne:ny+ne+nY] #Y
...
#            +(1-Tau_l)*W*Ucc*phi - Xi) 
    ...
    ret[19] = xi - Xi
    ret[20] = tauR_k + (k_*(r-R_)*Uc*mu + (1-eta)*(1-Tau_k_)*pi*Uc*mu)/(K_*(R_-1+delta)*EXi)
    ret[21] = tauR_k_ - EtauR_k
    ret[22] = tauR_l - (-(1+gamma)*W*l*Uc*mu)/YUc

    ret[23] = mu_ - EUc_mu/EUc #x_
    ret[24] = Alpha_ - beta*R_*m_*EUc#rho2_
    ret[25] = Alpha_ - beta*m_*EUc_r#rho3_
    ret[26] = Efoc_k_
...
def G(w):
    ...
    ret[7] = -W*Uc*phi
    ret[8] = logm
    ret[9] = TauR_k_ - tauR_k_
    ret[10] = YUc - W*l*Uc
    ret[11] = TauR_l-tauR_l
...
def f(y):
    ...
    '''
    logm,muhat,nu_a,logc,logl,r,rho2_,rho3_,phi,res,foc_k,foc_Tau_k,foc_Tau_k_,foc_R,foc_R_,xi,tauR_k,tauR_k_,tauR_l,x_,logk_,rho1_,alpha_= y
...
def Finv(YSS,z):
    ...
    '''
    logm,muhat,nu_a = z
    logXi,Tau_k_,logAlpha,logK_,R_,W,Tau_l,Psi,T,TauR_k_,YUc,TauR_l = YSS
    ...
    xi = np.ones(c.shape)*Xi    
    
    
    tauR_k = -(k_*(r-R_)*Uc*mu + (1-eta)*(1-Tau_k_)*pi*Uc*mu)/(K_*(R_-1+delta)*xi)
    tauR_l = (-(1+gamma)*W*l*Uc*mu)/YUc
    #logm,muhat,nu_a,logc,logl,r,rho2_,rho3_,phi,res,foc_k,foc_Tau_k,foc_Tau_k_,foc_R,foc_R_,x_,logk_,rho1_
    return np.vstack((
    logm,muhat,nu_a,np.log(c),np.log(l),r,rho2_,rho3_,phi,res,foc_k,foc_Tau_k,foc_Tau_k,foc_R,foc_R,xi,tauR_k,tauR_k,tauR_l,x_,np.log(k_),rho1_,alpha_
    ))
...
def GSS(YSS,y_i,weights):
    ...
    '''
    logm,muhat,nu_a,logc,logl,r,rho2_,rho3_,phi,res,foc_k,foc_Tau_k,foc_Tau_k_,foc_R,foc_R_,xi,tauR_k,tauR_k_,tauR_l,x_,logk_,rho1_,alpha_  = y_i
    logXi,Tau_k_,logAlpha,logK_,R_,W,Tau_l,Psi,T,TauR_k_,YUc,TauR_l  = YSS
    ...
    weights.dot(foc_R_),
    weights.dot(TauR_k_-tauR_k_),
    weights.dot(YUc-W*l*Uc),
    weights.dot(TauR_l-tauR_l)
    ]
    )
