...
ny = 40 # number of individual controls (m_{t},mu_{t},c_{t},l_{t},rho1_,rho2,phi,x_{t-1},kappa_{t-1}) Note that the forward looking terms are at the end
ne = 16 # number of Expectation Terms (E_t u_{c,t+1}, E_t u_{c,t+1}mu_{t+1} E_{t}x_{t-1 E_t rho_{1,t-1}} [This makes the control x_{t-1},rho_{t-1} indeed time t-1 measuable])
...
def F(w):
    ...
    ret[14] = foc_k - ( mu*Uc*((1-tau_k_)*(pi_k) - (R_)) + (1-tau_k_)*Uc*r_k/EUc_r *rho3_/beta 
                -fnk*phi2 -Xi_/beta + fk*Xi  ) #fock
    ret[15] = -phi2*fnn - Eta + Xi*fn  + mu*Uc*(1-tau_k_)*pi_n + (1-tau_k_)*Uc*fnk/(beta*EUc_r)*rho3_ #nl
    ret[16] = rho1*Uc + rho2hat/m + rho3hat/m  #logm
    ret[17] = rho2hat_ - R_ * rho2_ #temp1
    ret[18] = rho3hat_ - (1-tau_k_) *rho3_ #temp2
    ...
    ret[22] = labor_res - (l*w_e - nl )
    ret[23] = foc_W - ((1-tau_l)*w_e*l*Uc*mu + (1-tau_l)*phi1/W +phi2 ) #+(rho2_/beta -k_*EUc_mu - (1-tau_k)*rho3_/(Ri_*(Ri_-1))/beta+ Eta*b_)*psi_hat  )
    ret[24] = foc_tau_k - Efoc_tau_k
    ret[25] = res - (f - c - Gov - k)
    ret[26] = r_k - (A * xi_k*(xi_k-1) * k_**(xi_k-2) * nl**xi_l)
    ret[27] = dk_dtau_k - ( EUc_r/((1-tau_k_)*EUc_r_k) )
    ret[28] = tR_khat - mu*Uc*( (pi) - dk_dtau_k * ((1-tau_k_)*(pi_k) - (R_)))*(1-tau_k_)/(R_*dK_dtau_k*EXi)
    ret[29] = tW_khat - (dk_dtau_k * fnk * phi2) *(1-tau_k_)/(R_*dK_dtau_k*EXi)
    ret[30] = tE_khat - (dK_dtau_k * (Xi_/beta - EXi * R_) + dk_dtau_k*cov_Uc_r*EXi/EUc - dk_dtau_k*covXi_r)*(1-tau_k_)/(R_*dK_dtau_k*EXi)
    ret[31] = xi - Xi
    ret[32] = tR_k - EtR_khat
    ret[33] = tW_k - EtW_khat
    ret[34] = tE_k - EtE_khat
    ret[35] = dl_dtau - ( -Ul/((1-tau_l)*Ull) )
    ret[36] = tR_l  + ((1-tau_l)*W*w_e*mu*Uc*dl_dtau -W*w_e*l*Uc*mu)/dUc_dtau_l#+ (mu*Uc*w_e*W*(dl_dtau*(1-tau_l) - l)/dUc_dtau_l)
    ret[37] = tW_l - (W*Xi-Eta)*dL_dtau/dUc_dtau_l#(dl_dtau*W*w_e*(Uc-Xi))/dUc_dtau_l
    ret[38] = tE_l - ((dl_dtau*W*w_e*(Uc-Xi))/dUc_dtau_l)
    ret[39] = alpha_ - Alpha_
    
    
    ret[40] = Efoc_k #k_
    ret[41] = mu_  - EUc_mu/EUc #muhat
    ret[42] = Alpha_ - beta*R_*m_*EUc #rho2_
    ret[43] = Alpha_ - beta*(1-tau_k_)*m_*EUc_r #rho3
...
def f(y):
    ...
    '''
    logm,muhat,nu_a,nu_e,logc,logl,lognl,w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2_,rho3_,labor_res,res,foc_W,r_k,dk_dtau_k,tR_khat,tW_khat,tE_khat,tR_k,tW_k,tE_k,dl_dtau,tR_l,tW_l,tE_l,xi,x_,rho2hat_,rho3hat_,logk_,alpha_ = y
...
def Finv(YSS,z):
    ...
    rho1 = (Uc +mu*Ucc*( (1-tau_k)*(pi)-k_*(1/beta) + (1-tau_l)*W*w_e*l -(c-T) -Uc/Ucc ) + (1-tau_l)*Ucc/Uc*phi1 - Xi)/(Alpha_*Ucc/(beta*Uc) -Ucc*m)
    ...
    tR_khat = mu*Uc*( (pi) - dk_dtau_k * ((1-tau_k)*(pi_k) - (R_)))*(1-tau_k)/((R_)*dK_dtau_k*Xi)
    tW_khat = (dk_dtau_k * fnk * phi2) *(1-tau_k)/((R_)*dK_dtau_k*Xi)
    tE_khat = (dK_dtau_k * (Xi/beta - Xi * R_) )*(1-tau_k)/((R_)*dK_dtau_k*Xi) * np.ones(c.shape)
    ...
    #logm,muhat,nu_a,nu_e,logc,logl,lognl,w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2_,rho3_,labor_res,res,foc_W,r_k,dk_dtau_k,tR_khat,tW_khat,tE_khat,tR_k,tW_k,tE_k,dl_dtau,tR_l,tW_l,tE_l,xi,x_,rho2hat_,rho3hat_,logk_
    return np.vstack((
    logm,muhat,nu_a,nu_e,np.log(c),np.log(l),np.log(nl),w_e,r,pi,f,rho1,phi1,phi2,foc_k,foc_R,foc_tau_k,rho2,rho3,  labor_res,res,foc_W,r_k,dk_dtau_k,tR_khat,tW_khat,tE_khat,tR_khat,tW_khat,tE_khat,dl_dtau,tR_l,tW_l,tE_l,xi,x_,rho2hat_,rho3hat_,np.log(k_),alpha   
    ))
...
def GSS(YSS,y_i,weights):
    ...
    [weights.dot(K - k_), weights.dot(f - c - Gov - K), weights.dot(l*w_e - nl), weights.dot((pi)*mu*Uc + rho3_/beta), weights.dot(W*w_e*l*Uc*mu + phi1 ),
     R_-1/beta,weights.dot(foc_R), weights.dot((1-tau_l)*w_e*l*Uc*mu+ (1-tau_l)*phi1/W +phi2 ),T+Gov,weights.dot(B_-(x_*m/Alpha_ - k_)) ,
