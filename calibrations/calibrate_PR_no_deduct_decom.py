...
def F(w):
    ...
    ret[18] = foc_tau_k - (m_*rho3_*(fk)*Uc - mu*Uc*((1-xi_l)*f) )
    ret[19] = Efoc_tau_k - hatEfoc_tau_k
    ret[20] = xi - Xi
    ret[21] = dk_dtau_k_ - ( EUcfk/((1-tau_k_)*EUcfkk) )
    ret[22] = tR_khat - mu*Uc*((1-xi_l)*f - dk_dtau_k_ *( (1-tau_k_)*((1-xi_l)*fk) -delta_i - R_  ) )*(1-tau_k_)/((R_+delta_i)*dK_dtau_k_*EXi)
    ret[23] = tW_khat - (dk_dtau_k_ * flk * phi) *(1-tau_k_)/((R_+delta_i)*dK_dtau_k_*EXi)
    ret[24] = tR_k - EtR_k
    ret[25] = tW_k - EtW_k
...
def Finv(YSS,z):
    ...
    tR_k = mu*Uc*((1-xi_l)*f  - dk_dtau_k_ *( (1-tau_k)*((1-xi_l)*fk)-delta_i - R_  ) )*(1-tau_k)/((R_+delta_i)*dK_dtau_k_*Xi)
    tW_k = (dk_dtau_k_ * flk * phi) *(1-tau_k)/((R_+delta_i)*dK_dtau_k_*Xi)
